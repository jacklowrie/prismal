...
- `chkenv.sub`: Example job file, shows how to load the conda env. More of a "hello, world!".
...
On Gilbreth, the partition, memory, nodes, gpus-per-node, and account seem to be required.
Time defaults to 30min, but specifying is smart (shorter jobs might jump the queue, longer jobs need
the spec).
